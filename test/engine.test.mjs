...
test('RED: a script that quotes a whole header deep in its body is not a generated file', async () => {
  /* A test suite that builds a fixture header from a format string carries
     the marker AND `content-hash:` on one line, far below its first line.
     Read as a header, the `%s` is no hash, so the script was reported as a
     generated file that lost its hash, with `apply` as the fix, whenever the
     member's copy differed from the manifest. The generator only ever writes
     its header at the top, so that is the only place it is read. */
  const PATH = '06 AI Team/AI Team Knowledge/Scripts/check-the-shims.py';
  const SOURCE = '#!/usr/bin/env python3\n"""Checks the shims."""\n\nimport sys\n\n\ndef fixture(h):\n    return ("---\\nname: x\\n---\\n"\n            "<!-- GENERATED by scaffold-init.py content-hash:%s -->\\n" % h)\n\n\nprint(fixture("0" * 12))\n';
  assert.equal(await engine.generatedState(PATH, SOURCE, hash), null);
  const files = cleanFiles();
  files[PATH] = SOURCE;
  const r = await engine.runChecks({ fs: vault(files, cleanFolders), hash, remote: harnessManifest([[PATH, 'script']]), local: null, installedVersion: '1.5.0' });
  const f = r.findings.find((x) => x.path === PATH);
  assert.equal(f.kind, 'file', 'it is an ordinary file the three-way check judges');
  assert.ok(!f.message.includes('generated header'), f.message);
});

test('the header is still found everywhere the generator writes it, in every state', async () => {
  /* First line (TOML, GEMINI.md), after the frontmatter (skills, shims), and
     after one leading blank line (`.claude/settings.README.md`). */
  assert.equal((await engine.generatedState('GEMINI.md', GEMINI_REAL, hash)).state, 'intact', 'first line');
  assert.equal((await engine.generatedState('GEMINI.md', GEMINI_REAL + 'Always answer in German.\n', hash)).state, 'edited', 'first line, edited');
  assert.equal((await engine.generatedState(SKILL_PATH, SKILL_REAL, hash)).state, 'unhashed', 'after the frontmatter, no usable hash');
  const skill = SKILL_REAL.replace('PLACEHOLDER', '0123456789ab');
  assert.equal((await engine.generatedState(SKILL_PATH, skill, hash)).state, 'edited', 'after the frontmatter, hash that does not match');
  assert.equal((await engine.generatedState('.claude/settings.README.md', '\n' + GEMINI_REAL, hash)).state, 'edited', 'after a leading blank line');
  const files = cleanFiles();
  files[SKILL_PATH] = SKILL_REAL;
  const r = await engine.runChecks({ fs: vault(files, cleanFolders), hash, remote: harnessManifest([[SKILL_PATH, 'doc']]), local: null, installedVersion: '1.5.0' });
  const f = r.findings.find((x) => x.path === SKILL_PATH);
  assert.equal(f.kind, 'generated');
  assert.ok(f.message.includes('no content hash'), f.message);
});
